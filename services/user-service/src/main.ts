...
async function bootstrap() {
  ...
  app.getHttpAdapter().get('/health', (_req: unknown, res: { json: (v: unknown) => void }) => {
    res.json({ status: 'ok', service: 'user-service', uptime: process.uptime(), version: '0.1.0' });
  });

  await app.listen(env.PORT);
  logger.log(`🚀 user-service running on port ${env.PORT}`);

  setupGracefulShutdown(app, logger, 'user-service');
}
