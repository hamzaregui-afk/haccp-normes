...
export class JwtStrategy extends PassportStrategy(Strategy) {
  constructor() {
    super({
      ...
      // NB: audit WRITES are event-driven (tenantId comes from the domain event,
      // not the request) so this only re-scopes audit READS (/audit) — exactly
      // what the SUPER_ADMIN cockpit's audit page needs.
