...
| INFRA-1 | P0 | 🟢 résolu (2026-07-16) | API prod = IP brute + cert auto-signé | `https://178.105.126.165` + cert self-signed → devices rejettent → app inutilisable en prod. **Fait** : (1) **DNS ✅ 2026-07-16** — A `app → 178.105.126.165` créé chez Hostinger, vérifié (`app.normes-haccp.com` résout vers Hetzner, le gateway répond) ; apex/`www`/MX du site officiel intacts. (2) mobile pointé sur `https://app.normes-haccp.com` (`eas.json`+`client.ts`). (3) action **`setup-tls`** prête (Let's Encrypt/certbot, garde-fou DNS, renouvellement auto) ; nginx catch-all matche déjà le domaine. **TLS émis ✅** : action `setup-tls` exécutée (Deploy #269), certificat **Let's Encrypt** actif sur `app.normes-haccp.com` (CN=YE1, valide 16/07→14/10/2026, renouvellement auto), vérifié en **TLS strict** (login 401 / GET 200 sans `-k`). Sous-domaine retenu = **`app`** (reliquat `api` pointe encore chez Hostinger, inutilisé — à supprimer pour faire propre). Runbook : `MOBILE_DEPLOY.md`. |
...
| # | P | Statut | Sujet | Détail |
|---|---|---|---|---|
| PUSH-1 | P1 | 🟢 corrigé | Notifications push absentes | **Transport Expo Push** (validé). **Backend** (notification-service) : modèle `PushDevice` + migration ; `POST/DELETE /api/v1/notifications/devices` (JWT-scoped) ; `DeviceService`/`ExpoPushService` (POST `exp.host`, ne throw jamais, purge `DeviceNotRegistered`)/`PushService` ; push fire-and-forget sur `nc.created` (→ managers/quality) et `task.assigned` (→ assigné), gardé par le dedup existant ; env `EXPO_ACCESS_TOKEN` opt. WebSocket inchangé (push additif). **Mobile** : `expo-notifications` + plugin ; `lib/push.ts` (permission → ExpoPushToken → enregistrement ; désinscription au logout) ; branché dans `RootNavigator` (à l'auth) + `ProfileScreen` (logout). **Tests** : backend notif 4/39, mobile 9/145 ; `tsc` OK des deux côtés. *(Livraison device réelle nécessite les credentials EAS — cf. `MOBILE_DEPLOY.md`.)* |
| OFF-1 | P0 | 🟢 corrigé | Mode hors-ligne / sync inexistant | Approche React Query offline-first (validée PO). Ajout `@react-native-community/netinfo`, `@react-native-async-storage/async-storage`, `@tanstack/react-query-persist-client` + `query-async-storage-persister`. **Lectures** : `PersistQueryClientProvider` (AsyncStorage, `gcTime` 24 h) → agenda/checklist/sites dispo hors-ligne. **Écritures** : `NetInfo→onlineManager` ; les 2 mutations (checklist `PATCH controls/tasks`, NC `POST` + photos) déplacées dans `queryClient.setMutationDefaults` par **clé** (`lib/mutationKeys.ts`) → mises en pause hors-ligne, **persistées, rejouées** à la reconnexion / après redémarrage (`resumePausedMutations`). UX : soumission hors-ligne → alerte « Enregistré hors ligne » + `OfflineBanner` (hors-ligne / synchronisation…). i18n fr/en/ar. **Suite mobile 8/140 verte**, `tsc` OK. *(Rejeu réel validable sur build device.)* |
| NCPHOTO-1 | P1 | 🟢 corrigé (mobile) | Photos NC absentes | Ajout `expo-image-picker@~15.0.7` + plugin `app.json`. `NCFormScreen` : section Photos (caméra + galerie, max 5, miniatures + retrait, `quality 0.6`). À la soumission : `POST /nonconformities` → récupère l'`id` → upload séquentiel de chaque photo via `POST /nonconformities/:id/photos` (multipart, champ `file`). Échec photo **non bloquant** (NC déjà créée) → comptage + alerte partielle. i18n fr/en/ar. Tests : mock `expo-image-picker`, **suite mobile 8/139 verte**, `tsc` OK. *(Upload multipart RN/axios : validable définitivement sur build device.)* |
| OBS-1 | P2 | 🔴 ouvert | Pas de crash-reporting mobile | Aucun Sentry/observabilité côté app (backend = Prometheus/Grafana). |
| CI-1 | P1 | 🟠 documenté, bloqué credentials | Pas de pipeline EAS mobile | CI couvre lint/typecheck/test du workspace mobile, mais aucun job EAS build/submit. `eas.json` : `ascAppId` placeholder, `google-service-account.json` absent. **Prérequis non automatisables** (comptes/secrets) : login Expo, Apple Developer (`ascAppId`), Google Play (`google-service-account.json`). Commandes `eas build`/`eas submit` + qui-fait-quoi documentés dans `MOBILE_DEPLOY.md` (Étapes 3-4). |
...
| TEST-3 | P0 | 🟢 corrigé | Suites de tests **backend** cassées (12 services) | (1) `isolatedModules:true` + `jest-setup-env.ts` déjà en place (sessions préc.). (2) Assertions périmées : `where:{id,tenantId}` double-scope (delete/update), messages FR, mocks `@haccp/shared-utils` incomplets (`extractResourceId`/`publishDomainEvent`), `RmqContext` (`getChannelRef().ack`) pour les consumers audit/notification, forme **callback** de `$transaction`, fenêtre DLC `startOfDay→endOfDay` (~jours+1), `tracability` sans `jest.config.ts` (fallback babel-jest → 0 test). (3) **Clobber Prisma** : le monorepo partage un seul `@prisma/client` dans le store pnpm ; `prisma generate` d'un service écrase les enums runtime d'un autre (`TaskStatus` vs `NCSeverity`) → control/nonconformity/report jamais verts ensemble, et le job CI unit-tests ne générait aucun client. → 2ᵉ générateur `testClient` (output service-local) + `moduleNameMapper` `@prisma/client`→ce client dans `control` et `nonconformity` ; step `pnpm db:generate` ajouté au job CI. **Balayage complet : 12/12 services, 691 tests verts.** |
...
| # | Phase | Statut | Détail |
|---|---|---|---|
| PRINT-A0 | A — **DÉPLOYÉE EN PROD** | 🟢 fait | Phase A live sur Hetzner (`/api/v1/media-profiles`, `/printer-assignments`, `/resolve` = 401). Migration appliquée, **zéro régression** (tous les services existants 401/200). Blocages levés au passage : **SSH serveur** (reboot Hetzner), **guardrail cuid** (4 DTO), **reload gateway** (bind-mount fichier → force-recreate). |
